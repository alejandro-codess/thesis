...
## reading in 'income`
income <- read.delim(file.choose("input/income.txt"), header = TRUE)
...
ggplot(income, aes(x = workclass, fill = income)) +
  geom_bar(position = "fill") +
  facet_grid(. ~ sex) +
  scale_x_discrete("work sector") +
...
First, let's cut down the size of our complete data set a little, so that some functions later on run faster. We'll use `dplyr`'s `slice_sample()` function, which lets us automatically random sample 20% of our data set - see `?slice_sample()`:

```{r Creating a smaller data set.}
## sample 20% of the data in the 'adult' data set
set.seed(1)
income_frac = slice_sample(income, prop = 0.2)
```
...
# It is a statistical validity principal to select your data randomly, so in this manner we are avoiding any kind
# of correlation, patterns, generalization, and avoiding bias. By working with random selections we are trying to 
# avoid all this possible problems and seek to have the best possible model.



```

(4) Look at `?slice_sample` and find a way to sample an exact number of rows instead of a proportion. Create a new object called `income_1e4` which contains exactly 10.000 randomly sampled rows from `income`.
...
income_1e4 <- slice_sample(income, n = 10000)
income_1e4
...
new_tst = createDataPartition(y = tst_income$income, p = 0.40, list = FALSE)
fsplit_1 = tst_income[new_tst, ]
fsplit_2 = tst_income[-new_tst, ]

fsplit_1
fsplit_2

## answer check: nrow(fsplit_1) should be 1085, nrow(fsplit_2) should be 722 or 723
```
...
(6) Is it now clear what the `number` argument to `trainControl()` means? Create a new `"knn"` model that predicts `sex` based on all other variables, using `train()` and 10-fold cross validation (instead of the 5-fold used above).

```{r Exercise 6. Predict `sex` using 10-fold cross validation.}
## predict `sex` using 10-fold cross validation
set.seed(1)
sex_knn = train(sex ~ ., method = "knn", data = trn_income,
...
plot_knn_results <- function(fit_knn) {
  ggplot(fit_knn$results, aes(x = factor(k), y = Accuracy)) +
    geom_bar(stat = "identity") +
    scale_x_discrete("value of k", limits = factor(fit_knn$results$k)) +
...
Note how we have to turn k into a factor (in 2 spots), otherwise ggplot sees it as a continuous variable instead of a categorical variable. 
...
# Using some bigger slice from the dataset with some different random sample
set.seed(42)
new_income_frac = slice_sample(income, prop = 0.3)

#Using the different random sample and a different data partition.
set.seed(42)
new_trn_index = createDataPartition(y = new_income_frac$income, p = 0.83, list = FALSE)
new_trn_income = new_income_frac[new_trn_index, ]
new_tst_income = new_income_frac[-new_trn_index, ]

new_income_knn = train(income ~ ., method = "knn", data = new_trn_income,
...
set.seed(42)
ocu_income_frac = slice_sample(income, prop = 0.4)


#Using the different random sample and a different data partition.
set.seed(42)
ocu_trn_index = createDataPartition(y = ocu_income_frac$income, p = 0.90, list = FALSE)
ocu_trn_income = ocu_income_frac[ocu_trn_index, ]
ocu_tst_income = ocu_income_frac[-ocu_trn_index, ]

best_k = train(occupation ~ ., method = "knn", data = ocu_trn_income,
  trControl = trainControl(method = 'cv', number = 5))
best_k
...
set.seed(1)
income_lgr_f2 = train(native.country ~ ., method = "glm",
  family = binomial(link = "logit"), data = trn_income_f,
  trControl = trainControl(method = 'cv', number = 5))
...
' income_knn for k = 5 reaches accuracy of 0.829, whereas income_knn_as 
for k = 7 an accuracy of 0.75 so then we can say income_knn with more
variables into account performs better.'
...
income_lgr_asm2 = train(income ~ . - capital.gain,
  method = "glm", data = trn_income_f, family = binomial(link = "logit"),
  trControl = trainControl(method = 'cv', number = 5))

income_lgr_asm2

' capital.loss column was not found by R when added after capital.gain 
in the code.'
```
...
set.seed(1)
income_lgr_asm3 = train(sex ~ (education.num + hours.per.week + income)^2,
  method = "glm", data = trn_income_f, family = binomial(link = "logit"),
  trControl = trainControl(method = 'cv', number = 5))
...
' For the income_knn model we got a k = 3 accuracy of 0.827 which is the 
best performance of the two, whereas in the income_knn_pP model our
best accuracy was for k = 5 with 0.816.'
```
...
income_knn_tG_2 = tuneLength(occupation ~ ., method = "knn", data = trn_income,
  trControl = trainControl(method = 'cv', number = 5),
  tuneGrid = data.frame(k = c(3, 12)))
...
set.seed(1)
sex_knn_mR = train(sex ~ ., method = "knn", data = trn_income,
  trControl = trainControl(method = 'cv', number = 5, classProbs = TRUE,
...
The actual 'confusion matrix' is at the top. The columns represent the actual classes in the test data, while the rows show the classes we predicted.  It shows us that (your numbers might differ a bit): of the 1807 people in the test set, 261 + 189 actually made over 50k (column 1), and of those 450 people, we predicted 261 correctly as making over 50k (top-left value). Conversely, 1357 made less than 50k in reality (column 2), and of those, 1233 were correctly predicted as such (bottom-right value).
...
It appears that our effort to train on 'recall' has had an effect! Looking at the `confusionMatrix()` output, we can see that the original accuracy-trained `"knn"` had a recall of 0.5388, while the recall-trained `"knn"` had a recall of 0.5806 - more of the high-earners have been correctly classified as such.
...
set.seed(1)
predicted_outcomes_mR <- predict(income_knn_mR, trn_income)

knn_mR_confM_2 <- confusionMatrix(predicted_outcomes_mR, trn_income$income)
knn_mR_confM_2
...
predicted_outcomes_mR <- predict(income_lgr, tst_income)

knn_mR_confM_3 <- confusionMatrix(predicted_outcomes_mR, tst_income$income)
knn_mR_confM_3

' Accuracy for glm = 0.846 and for the knn = 0.827, both for the test
sets, so general linear model logit performes better in this dataset.'
...
"income_lgr_asm18 = train(income ~ . - native.country,
  method = "#glm", data = trn_income_f, family = binomial(link = logit),
  #trControl = trainControl(method = 'cv', number = 3),
  #tuneGrid = data.frame(k = c(3, 5, 7, 9, 11, 13, 15)))"

income_lgr_asm18 = train(income ~ - native.country, method = "knn", data = trn_income,
  trControl = trainControl(method = 'cv', number = 3, classProbs = TRUE,
  tuneGrid = data.frame(k = c(3, 5, 7, 9, 11, 13, 15)),
  summaryFunction = prSummary, metric = "Precision"))

  ?train
#I could not move forward with this code at this point, I couldn not see why
# since income is a column in my dataset and did not continue with the ggplot 
# lines.
...
slice_sample()          | sample random rows from a data frame, by proportion or number
